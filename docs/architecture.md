...
- The embedded SPA also exposes multi-stage quota reset approvals for user subscription groups. Ordinary users request a reset for one current active subscription group with a reason. For a v2 request, `backend/internal/quotareset` snapshots one initial logical node from all current direct department memberships, resolves each department by exact external id without walking ancestors, prefers valid configured approvers, and falls back only to synchronized representatives of that same department. It then appends the enabled ordered department chain configured for the selected provider and subscription group. One eligible approver or an admin fallback can satisfy only the current node; an approval is reused for every later node whose immutable approver snapshot contains the same actor, including non-adjacent nodes, while queued and automatically satisfied nodes remain non-actionable. Every v2 approval or rejection requires a durable comment. Legacy v1 requests retain their original single-stage behavior. Only after all required nodes are satisfied does the workflow call the primary relay provider through `relay.UserSubscriptionQuotaResetter` for the requester's relay user and selected group; AI Efficiency never accesses the relay database directly. Normalized approval-chain, chain-node, request-node, request-node-approver, and request-decision rows plus request events preserve configuration and immutable audit snapshots. `/api/v1/work-items/counts` and quota-reset queue permissions are computed from the actionable current node, including admin fallback and reset-retry ownership, rather than from future queued nodes. Under Organization & Login, admins configure department approvers, subscription-group chains, and an explicit notification channel. Notifications are rendered from a channel-neutral context by registered `wecom_group_robot` Markdown and `generic_webhook` versioned-JSON adapters; Enterprise WeChat delivery mentions current approvers only when synchronized recipient ids are available and reports missing mention coverage without exposing webhook secrets.
...
- `My Work`: `/`, `/work-items`, `/usage`, `/user`, and `/events` provide the ordinary company-user path for personal AI usage, pending work, setup, and readable usage records. `/` redirects to the personal AI Usage page at `/usage`, which now stays focused on usage and quota visibility instead of rendering AI access/setup guide cards. Missing reusable AI access is represented as an `ai_access_setup_count` item in `/work-items`, with `/user` as the detail/remediation target. `/work-items` is the shared pending-work entry and aggregates personal AI access setup, current-node quota reset approval or retry work, and admin-only directory offboarding candidates. The sidebar shows a hidden-when-zero `total_count` badge sourced from `/api/v1/work-items/counts`; quota-reset counts include only requests the viewer can act on now, exclude future queued and prior-approval-satisfied nodes, and deduplicate the admin total as personal AI access setup plus admin current-node fallback/retry work plus offboarding count. The embedded usage snapshot also carries a degradable `group_quotas` section derived from the current user's reusable API keys plus their bound group limits. `Usage Records` and `Code Repositories` remain secondary drill-down paths rather than the homepage's primary narrative.
- `AI Usage`: `/usage` is always the current user's personal AI Usage page and does not expose a choose-person dropdown. Its top tab strip always exposes `/usage/quota-reset`, where users can track their own reset requests, handle requests assigned to them at the active workflow node, inspect immutable node/decision history, and, for admins, inspect all requests as a current-node fallback queue. Approval and admin badges use the backend Work Items current-action counts; future queued approvers do not receive a badge or actionable request. Representatives use `/usage/team` for subtree-wide trend and member ranking, then open `/usage/members/:user_id` for a focused scoped-member detail page with subscription-group quota rows and delegated multiplier controls. `/usage/team` does not show quota cards, subscription quota rows, or multiplier controls.
...
| Quota reset approvals | `backend/internal/quotareset` | Owns v1 compatibility and the normalized v2 workflow: exact-direct-department initial resolution, validated configured approvers and same-department representative fallback, provider/group approval chains, immutable request-node and approver snapshots, durable decisions, current-node permissions and counts, approval reuse, reset/retry transitions through `relay.UserSubscriptionQuotaResetter`, lifecycle events, explicit notification channel registration, Enterprise WeChat Markdown mentions, generic versioned JSON, and redacted delivery metadata |
| Work items | `backend/internal/workitems` | Auth-scoped pending work counters for sidebar and `/work-items`, delegating quota-reset actionability to the quota-reset workflow module so only current-node approval/admin fallback and eligible retry work is counted, alongside best-effort relay-derived personal AI access setup and directory offboarding counts that remain available when the relay lookup fails |
...
| API surface | `backend/internal/handler`, `backend/internal/middleware` | HTTP handlers, routing, auth middleware, settings endpoints, representative `/user/team-usage/*` endpoints, node-aware quota reset user/admin decisions, approver candidates, approval-chain options/configuration, redacted notification settings, work item counts, admin team-usage audit, admin-users direct relay-user disablement/subscription jobs, and admin directory sync/offboarding endpoints |
