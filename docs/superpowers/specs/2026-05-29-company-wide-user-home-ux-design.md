...
1. 登录后默认入口改为面向普通使用者的 **我的 AI 使用中心**，先回答“我是谁、接入是否正常、本周有什么结果、下一步做什么”。
2. 保留当前研发自助接入能力，但把 CLI 命令、provider group、API key、doctor 等技术细节放进 `我的接入` 或高级区块。
3. 把原始 `Events` 从默认技术名调整为更友好的 `使用记录`，默认展示可理解摘要，原始事件细节通过高级展开或详情抽屉进入。
4. 把管理员配置收敛到 `管理后台` / `专家模式`，避免普通用户首屏看到 SCM Providers、Credentials、Relay Providers、Deployment、LDAP 等配置域。
5. 改善响应式 shell：桌面端可保留侧栏，平板和手机使用顶部栏 + 抽屉导航，避免固定 240px 侧栏压缩内容。
6. 建立统一的空状态、加载状态、错误状态和权限受限状态，让无数据用户也能知道下一步。
7. 使用更清晰的业务化术语，但不破坏代码和 API 内部的 provider / event / repo / credential 命名。
...
`/` 从当前技术 Dashboard 调整为普通用户首页：`我的 AI 使用中心`。
...
| Current Label | New Label | Default Audience | Notes |
| --- | --- | --- | --- |
| Dashboard | 我的 AI 使用中心 | 所有用户 | `/` 默认首页 |
| User | 我的接入 | 所有用户，研发优先 | 保留 CLI setup 和 provider group credential |
| Events | 使用记录 | 所有用户 | 默认摘要化，原始字段进入高级详情 |
| Repos | 代码仓库 | 研发、负责人、管理员 | Repo detail 继续承载 PR usage 和 freshness |
| Users | 用户管理 | 管理员 | 只对 admin 可见 |
| Settings | 管理后台 | 管理员 / 专家 | 聚合 provider、credential、relay、LDAP、deployment |

第一版不要求新增独立 `团队看板` 路由。如果现有数据不足，可把团队/全局指标保留在管理员可见的 `管理后台` 或后续 spec 中。
...
4. `下一步`
   - 未接入：进入 `我的接入`，按机器级和 repo 级步骤完成配置。
   - 有接入但无数据：运行 `ae-cli doctor`，检查 hooks 和最近 commit。
   - 有数据但未关联 repo/PR：进入 `代码仓库` 或查看 `使用记录` 的绑定状态。
   - 数据正常：查看本周趋势或最近记录。
5. `最近使用记录`
...
`/user` 调整为 `我的接入`，保留现有 `/user` self-serve 合同：
...
| --- | --- | --- |
| Dashboard | 我的 AI 使用中心 | 默认首页 |
| User | 我的接入 | CLI 和工具接入 |
| Events | 使用记录 | 摘要优先，原始事件高级可见 |
| Repos | 代码仓库 | repo/PR 使用状态 |
| Providers | AI 服务 | 管理后台或高级上下文使用 |
| Credentials | 密钥 / 凭据 | 默认隐藏在高级区 |
| Group | 可用分组 | 仅在接入或管理语境解释 |
| Binding Status | 关联状态 | 用状态说明解释 |
| Doctor | 诊断 | 命令仍显示 `ae-cli doctor` |

代码和 API 命名不必同步改成中文。实现应优先在组件文案和页面标题层完成术语转换，避免大范围重命名内部类型。
...
- 有使用但未关联 repo：运行 `ae-cli doctor`，查看 `使用记录` 的关联状态。
...
这些内容应通过 `高级`、`详情`、`管理后台` 或 admin-only 页面访问。
...
1. `/` renders `我的 AI 使用中心` as the authenticated default route.
2. Sidebar / mobile navigation show `我的 AI 使用中心`、`我的接入`、`使用记录`、`代码仓库` and hide admin entries for non-admin users.
3. Admin users can still see `用户管理` and `管理后台`.
...
- `/user` as `我的接入`, preserving the current CLI setup and provider/group credential contract.
- `/events` as user-facing usage records with advanced raw detail.
- admin-only `管理后台` / expert surfaces.
...
3. Reword `/user` into `我的接入` while preserving its current CLI and credential behavior.
4. Reframe `/events` as `使用记录`, keeping raw details available.
5. Group admin settings under `管理后台`.
