...
- Team summary and trend are served independently from the compatibility overview, each with its own authoritative origin, versioned Redis lane, lease, freshness, and stale-if-error boundary. Summary loads only aggregate stats; Trend loads only the stats and trend points needed for the complete team-total plus bounded Top 12 member and department-comparison series. Summary, trend, members, organization, and compatibility requests start independently with the same normalized range; a delayed or failed request does not remove successful siblings. The trend chart stays behind an async component boundary until a Trend response renders. Members and organization temporarily retain compatibility-overview projection, and the compatibility route retains its complete historical JSON shape plus `Deprecation`, `Sunset`, and successor `Link` headers for the compatibility release.
- Team members are served as an independent snapshot-bound ranking page, defaulting to 50 rows and capped at 100. Next/Previous cursor navigation restarts only the members section after `snapshot_expired`; the compatibility response temporarily retains only the organization tree for the current frontend.
...
three current read-model lanes. Summary and Trend each have an independent typed
origin and cache; split members, split organization, and the one-release
compatibility overview adapter temporarily retain the authorized overview generation:
...
- Each Redis key hashes deployment namespace, provider id and persisted
  ...
  Summary, Trend, and overview use distinct versioned key spaces, so no payload can
  satisfy another lane.
...
- A Summary value contains only a typed normalized window and summary aggregate.
  ...
  `top_member_trend`, and `department_trend`. An overview value contains the
  schema-versioned reconstructible `OverviewResponse` used by members,
  organization, and compatibility projections. None contains a request id, JWT,
  token-revocation state, provider credential, quota fact, or mutation decision. Request middleware owns
...
- The retained overview generation aggregates selected-window
  ...
  Members, organization, and compatibility overview call the same internal overview
  snapshot service and never make internal HTTP calls.
...
| Representative scope and team usage | `backend/internal/representativescope`, `backend/internal/readcache`, `backend/internal/teamusage` | Resolve representative subtree scope from current directory metadata and member-department memberships, derive and twice-check opaque scope versions, reuse namespace/provider/actor/scope/range-isolated Redis read models with bounded stale-if-error and authoritative fallback, serve Summary and bounded Trend from independent typed lanes while snapshot-bound paged members, shallow paged organization branches, and the legacy adapter temporarily share the overview generation, enforce delegated subject visibility and ancestor-only multiplier policy, and persist local `team_usage_rate_multiplier_audits` |
