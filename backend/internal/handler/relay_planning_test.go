...
func (p *relayPlanningSearchProvider) ListAccountsForPlatform(context.Context, string) ([]relay.Account, error) {
	return append([]relay.Account(nil), p.accounts...), nil
}

func (p *relayPlanningSearchProvider) SetAccountGroupRelationship(_ context.Context, accountID, groupID int64, _ []relay.AccountGroupRelationship, desiredPriority *int) error {
	p.accountUpdates++
	priority := 0
...
func TestRelayPlanningAdoptCurrentAccountsInitializesDesiredStateWithoutRelayWrite(t *testing.T) {
	...
	provider := &relayPlanningSearchProvider{
		...
		accounts: []relay.Account{
			{ID: 11, Name: "Account Alpha", Platform: "openai", Type: "oauth", Status: "active", Schedulable: true, GroupRelationships: []relay.AccountGroupRelationship{{GroupID: 999, Priority: 1}, {GroupID: 101, Priority: 2}}},
	...
	if listed.AccountManagementInitialized || len(listed.AccountPools) != 1 || listed.AccountPools[0].Drift {
