...
sequenceDiagram
    ...
    CLI->>WS: install global hooks + shadow baseline when required
...
- Attribution cutover protocol:
  One backend-owned runtime contract supplies `ledger_epoch`, `v1_write_policy`, and `minimum_cli_version` to installation enrollment, v1 ingest, v2 ingest/ACK, pool materialization, and the Activity/readiness epoch gate. Its pre-cutover default is `shadow_v2 + accept` with no minimum version. Supported `upgrade_required` combinations require a non-empty minimum CLI version; invalid combinations fail router startup. The same server configuration owns `cutover_at`: it is empty in shadow mode and must be one explicit RFC3339 UTC `Z` instant in formal mode, otherwise startup fails before routing. It is deliberately not advertised as a fourth client protocol field. Under `upgrade_required` both v1 bucket and revision writes return structured HTTP 409 without mutation, while the v2 endpoint continues to ACK the advertised epoch. Claim groups and durable pools inherit the contract epoch, and the pool canonical identity includes that epoch so identical shadow/formal accounting dimensions remain isolated. Docker/config examples expose all cutover-owned settings; changing them remains part of the separately authorized cutover runbook. The same attribution configuration exposes `setup_available` and `readiness_available`, both false by default; readiness without setup or outside the formal v2 epoch fails startup instead of creating a second rollout truth source.
- Current Activity surface:
  `/activity` remains the canonical personal AI Coding Activity surface, with authorized member drill-down at `/activity/members/:user_id` and representative/Admin organization views at `/activity/teams` and `/activity/teams/:team_id`. The legacy v1 routes remain unchanged before cutover. The backend also exposes authorization-revalidated `/api/v1/activity/v2/overview`, `/v2/repositories`, and `/v2/pull-requests` contracts for exact local-date ranges. These v2 reads aggregate compact pools in PostgreSQL, use fixed 20-row signed keyset pages, keep claim and SCM-sync coverage separate, and obtain the denominator only through Personal/Team Usage business services. The production router derives their epoch from the single validated attribution protocol only when it is `formal_v2`; before cutover the Activity epoch remains empty, so `shadow_v2` pools cannot change current Activity or readiness. Adjacent comparison eligibility uses the frozen server `cutover_at`, never `MIN(bucket_start_utc)`, so a complete zero-data period after cutover remains comparable while a period crossing the boundary is omitted. When explicitly enabled after formal cutover, `/auth/me` advertises setup/readiness capabilities and authenticated `GET /api/v1/attribution/status` combines aggregate installation state with that same Activity formal-pool predicate. The no-store status DTO has five user-level states and an active-only latest accepted timestamp, exposes no device metadata, and returns a local retryable failure when the PostgreSQL readiness read fails.
- Repository surface:
