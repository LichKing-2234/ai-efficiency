...
| Directory sync | `backend/internal/directorysync` | Configurable HTTP directory DSL validation/execution, bounded exact-department representative metadata append overrides, current department/member/membership facts, scheduled transactional apply runs, shared bounded offboarding count/page anti-join, and confirmed relay-user disable plus tx-aware token/revision finalization |
