...
- The backend owns durable repo configuration, reporting installations, hot v2 claims, HTTP Relay reconciliation, direct WebSocket aggregate materialization, epoch-isolated formal usage pools and commit relations, checkpoint-backed allocation authorization, Activity reads, legacy PR usage snapshots, and SCM/webhook handling. Phase 2 source code no longer serves v1 bucket/revision/report, legacy Activity, or AE OTLP contracts; their PostgreSQL tables and installation columns remain unused only for rolling-deployment rollback until Phase 3.
...
long-lived pools. It has no long-lived local daemon. Phase 2 source code has
removed the v1 bucket/revision/report, legacy Activity, and AE OTLP surfaces;
the old database objects remain unused until the separately gated Phase 3
schema contraction. Production runs `v0.1.0-preview.89` from commit `319735ac`
as Helm revision 89; both backend and prewarmer roles use the same image digest.
...
flowchart LR
    ...
    subgraph Backend["ai-efficiency backend"]
        Install["reporting_installations<br/>reporter auth; legacy columns unused until Phase 3"]
