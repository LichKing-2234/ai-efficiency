...
describe('command step helpers', () => {
  ...
  test('uses shared row primitives for step layout and copy affordance', () => {
    ...
    expect(source).toContain("font-semibold text-[11px] text-[var(--ink-4)]")
  ...
  test('keeps the setup workflow row on the denser reference shell', () => {
    ...
    expect(source).toContain("label ? <span className='shrink-0 font-semibold text-[11px] text-[var(--ink-4)]'>{label}</span> : null")
