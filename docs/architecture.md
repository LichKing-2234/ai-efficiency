...
| Data access | `frontend/src/api`, `frontend/src/stores` | Backend API clients, representative team-usage clients, paginated Directory clients, the generation-safe Work Items count store with completion-based 20-second freshness, and the shared Settings credential/Directory-source owner with five-minute reuse, request deduplication, auth-session reset/invalidation, and serialized mutation refresh |
