...
func (s *Service) GroupPoolUsage(ctx context.Context, request Request) (*GroupPoolUsageResponse, error) {
	...
	subject, err := s.resolveSubjectWithOptions(ctx, request.UserID, false)
	if err != nil {
		return nil, err
	}
	if !subject.configured {
...
func (s *Service) resolveSubject(ctx context.Context, userID int) (*resolvedSubject, error) {
	return s.resolveSubjectWithOptions(ctx, userID, true)
}

func (s *Service) resolveSubjectWithOptions(ctx context.Context, userID int, requireOrigin bool) (*resolvedSubject, error) {
