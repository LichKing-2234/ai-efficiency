...
- Remaining direction:
  finish T16's ordinary-workflow adoption gates and #252 only after its seven-day stable window, evaluate compact-trigger retention/claim deadlines from production distributions, migrate Claude/Kiro to the generic pool contract, and decide whether delivery-effectiveness correlation is useful. Cost allocation and individual ranking remain out of scope.
- Active Codex v2 contract:
  [`2026-08-11-codex-commit-token-attribution-v2-design.md`](./superpowers/specs/2026-08-11-codex-commit-token-attribution-v2-design.md) defines mutually exclusive HTTP `relay_official` and WebSocket `codex_local` claim groups, deterministic commit proof, 90-day hot detail, long-lived globally deduplicated usage pools, Usage-backed Activity ratio reads, in-page Repository/PR analysis filters, and administrator-only Repository operations. Production selected the formal HTTP contract at the verified 2026-08-12 cutover and added the WebSocket path in platform `v0.1.0-preview.85`. CLI-only `ae-cli/v0.2.0-preview.10` completed the current Codex 0.147 trusted-log, dual-baseline, and stale scan-progress repairs; its retained same-session canary materialized one direct formal pool with 8 responses, exactly `284,666` Token, zero gaps, and zero Request claims. Production remains on platform `v0.1.0-preview.85` / Helm revision `85`, and pre-cutover `shadow_v2` pools remain isolated.
