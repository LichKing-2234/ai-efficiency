...
defineProps<{ steps: QuotaResetWorkflowStep[] }>()
const { t } = useI18n()
</script>
<template>
  <ol class="divide-y divide-slate-200 border-y border-slate-200" data-testid="quota-reset-workflow-timeline">
    <li v-for="(step, index) in steps" :key="`${index}-${step.label}`" class="grid gap-2 py-3 sm:grid-cols-[2rem_minmax(0,1fr)_auto]">
      ...
      <span :class="['h-fit rounded-full px-2 py-1 text-xs font-medium', step.status === 'active' ? 'bg-cyan-50 text-cyan-800' : 'bg-slate-100 text-slate-600']">
        {{ t(`quotaReset.workflowStatus.${step.status}`) }}
      </span>
    </li>
  </ol>
</template>
