...
defineProps<{ steps: QuotaResetWorkflowStep[]; currentStep?: number }>()
const { t } = useI18n()
</script>
<template>
  <ol class="divide-y divide-slate-200 border-y border-slate-200" data-testid="quota-reset-workflow-timeline">
    <li v-for="(step, index) in steps" :key="`${index}-${step.label}`" class="grid gap-2 py-3 sm:grid-cols-[2rem_minmax(0,1fr)_auto]">
      ...
      <div class="min-w-0">
        ...
        <div v-if="step.decision" class="mt-1 text-sm text-slate-600"><span class="font-medium">{{ step.decision.actor_display_name || `User #${step.decision.actor_user_id}` }}</span><span> · {{ step.decision.comment }}</span></div>
        <div v-else-if="step.admin_fallback" class="mt-1 text-xs text-amber-700">{{ t('quotaReset.adminFallback') }}</div>
      </div>
      <span :class="['h-fit rounded-full px-2 py-1 text-xs font-medium', index === (currentStep ?? 0) && step.status === 'active' ? 'bg-cyan-50 text-cyan-800' : 'bg-slate-100 text-slate-600']">{{ t(`quotaReset.workflowStatus.${step.status}`) }}</span>
    </li>
  </ol>
</template>
