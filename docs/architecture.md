...
- `Relay Planning`: `/admin/relay-planning` is an explicit admin-only department x Platform workflow. It previews 30-day usage, global Token rank, source-group eligibility, least-cost allocation, and warnings before a second confirmation duplicates/activates relay Groups, assigns subscriptions, moves AE-managed API Keys, and removes source subscriptions. Successful results persist the current `relay_group_mappings` snapshot keyed by stable Group IDs; list/rebind/replan endpoints support explicit drift handling and mark surplus Groups inactive after member restoration. There is no background synchronizer or direct sub2api database coupling.
