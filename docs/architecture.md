...
- Team summary, trend, and members are served independently from the compatibility overview, each with its own authoritative origin, versioned Redis lane, lease, freshness, and stale-if-error boundary. Summary loads only aggregate stats; Trend loads only the stats and trend points needed for the complete team-total plus bounded Top 12 member and department-comparison series; Members loads complete-range batch stats in at-most-100-user chunks and ranks immutable rows once without acquiring the Trend capability or constructing an organization tree. Summary, trend, members, organization, and compatibility requests start independently with the same normalized range; a delayed or failed request does not remove successful siblings. The trend chart stays behind an async component boundary until a Trend response renders. Organization temporarily retains compatibility-overview projection, and the compatibility route retains its complete historical JSON shape plus `Deprecation`, `Sunset`, and successor `Link` headers for the compatibility release.
...
- `backend/internal/adminusers` owns one source-scoped `navigation_departments` SQL projection used by targets, list count/page, page enrichment, department options, immediate children, and summaries. It reads `directory_departments.effective_parent_external_id` exactly as persisted by the successful Directory Sync apply; administrator requests no longer count the source, walk every raw parent path, or infer missing-parent and cycle-anchor repairs. The same effective-subtree predicate and target reader back visible pages plus persisted and compatibility current-filter mutations. Only requested subtree, page-candidate ancestor, and requested-summary descendant relations remain recursive, each bounded to its result role; #171 will remove the transitional helper name and positional-placeholder coupling. Deployments upgrading existing directory snapshots must complete a successful apply with effective-hierarchy storage available before activating these readers because `NULL` is both the valid persisted root and the legacy column value, so request paths cannot safely infer a backfill. Representative-scope hierarchy reads remain unchanged in this ticket. Department options default to 20 and cap at 100; immediate-child navigation defaults to 25 and caps at 100. Page enrichment stays bounded by the 100-user page and its candidate-department/ancestor closure, and the frontend mounts one responsive row tree rather than duplicate mobile and desktop records.
...
four current read-model lanes. Summary, Trend, and Members each have an independent
typed origin and cache; split organization and the one-release compatibility overview
adapter temporarily retain the authorized overview generation:
...
- Each Redis key hashes deployment namespace, provider id and persisted
  ...
  Summary, Trend, Members, and overview use distinct versioned key spaces, so no
  payload can satisfy another lane.
...
- A Summary value contains only a typed normalized window and summary aggregate.
  ...
  normalized window and complete immutable ranked member rows. An overview value
  contains the schema-versioned reconstructible `OverviewResponse` used by
  organization and compatibility projections. None contains a request id, JWT,
...
- Summary calls only the Relay summary capability with the normalized range,
  ...
  members, project chart series, or construct an organization tree. Summary and
  Members set `RequireCompleteRange`; the Relay compatibility adapter may then
...
- The retained overview generation aggregates selected-window
  ...
  semantics, but compatibility reads never populate or consume the Trend cache.
  Organization and compatibility overview call the same internal overview snapshot
  service and never make internal HTTP calls.
...
| Administrator users | `backend/internal/adminusers`, `backend/internal/adminsubscription` | Persisted source-scoped effective-department projection and effective-subtree-to-user eligibility shared by targets, count/page, page-local enrichment, bounded options, immediate-child navigation, and summaries; one current-filter target reader preserves list, persisted-job, and compatibility-batch predicate parity |
| Representative scope and team usage | `backend/internal/representativescope`, `backend/internal/readcache`, `backend/internal/teamusage` | Resolve representative subtree scope from current directory metadata and member-department memberships, derive and twice-check opaque scope versions, reuse namespace/provider/actor/scope/range-isolated Redis read models with bounded stale-if-error and authoritative fallback, serve Summary, bounded Trend, and snapshot-bound paged Members from independent typed lanes while shallow paged organization branches and the legacy adapter temporarily share the overview generation, enforce delegated subject visibility and ancestor-only multiplier policy, and persist local `team_usage_rate_multiplier_audits` |
