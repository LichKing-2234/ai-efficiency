...
- Relay Planning persistence now includes independent Relationship Operation, affected-Mapping ownership, immutable directional step, and attempt tables, plus a Mapping baseline revision. Active ownership is partial-unique per Mapping. These tables are storage-only until persist-before-dispatch orchestration replaces legacy `operation_state` as the runtime source of truth.
