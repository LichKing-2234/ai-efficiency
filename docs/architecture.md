...
- `Relay Planning`: `/admin/relay-planning` is an explicit admin-only department x Platform workflow. Its initial preview automatically uses the recommended Group count, capped at the number of indivisible eligible members; explicit Group-count overrides are reserved for replanning an existing mapping. The preview shows 30-day usage, global Token rank, source-group eligibility, least-cost allocation, expected relay copy names, and warnings before a centered second confirmation duplicates/activates relay Groups, assigns subscriptions, moves AE-managed API Keys, and removes source subscriptions. The read-only preview reuses a valid schema-v3 Team Usage prewarm generation for provider-wide 30-day usage and falls back to the exact Relay sources when that generation is absent or invalid; current subscription membership and migratable API Key facts remain authoritative per candidate. Relay-returned IDs and names replace the preview targets after execution. Successful results persist the current `relay_group_mappings` snapshot keyed by stable Group IDs; list/rebind/replan endpoints support explicit drift handling and mark surplus Groups inactive after member restoration. There is no background synchronizer or direct sub2api database coupling.
