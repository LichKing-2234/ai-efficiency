...
**Follow-up Note 2026-06-09:** Bitbucket Server PR webhook payloads can omit top-level `repository` while still carrying repository metadata under PR refs. The inbound handler and Bitbucket parser now resolve repository identity from top-level `repository`, then `pullRequest.toRef.repository`, then `pullRequest.fromRef.repository`, so valid PR merge events no longer fail early with `missing repository info`. Backend tests passed with `AE_TEST_POSTGRES_DSN='postgres://postgres:postgres@127.0.0.1:15432/postgres?sslmode=disable' go test ./...`.
