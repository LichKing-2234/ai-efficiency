...
- `Relay Planning`: `/admin/relay-planning` is an explicit admin-only department x Platform workflow. Its initial preview automatically recommends a fixed target Group count from indivisible source members and keeps Template Group (copy configuration) separate from Migration Source Group (old subscription/API Key migration). The preview exposes 30-day usage, global Token rank, source eligibility, target assignment editing, an explicit unassigned area, expected copy names, and read-only mapping warnings before a centered second confirmation. Confirm duplicates/activates Template-based Groups, applies only the final assignment matrix, migrates source members, and treats explicitly added non-source users as new bindings without source cleanup. Replan is entered from one mapping, preserves stable target Group IDs, recommends same-department source members that fit each target's remaining planning target, and never resizes or silently reshuffles Groups. Mapping reads mark unavailable Groups, unmanaged Relay members, relationship drift, and cross-mapping conflicts; repairs and rebinding change only the mapping until Confirm. Unmanaged members count toward target capacity and can be explicitly adopted as target-only bindings. Each mapping retains per-Group/member operation state and errors so failed steps remain retryable. The read-only preview reuses a valid schema-v3 Team Usage prewarm generation for provider-wide 30-day usage and falls back to exact Relay sources when that generation is absent or invalid; current subscription membership and migratable API Key facts remain authoritative per candidate. Relay-returned IDs and names replace preview targets after execution. Successful results persist current `relay_group_mappings` snapshots keyed by stable Group IDs, assignments, source relationships, and retryable operation state. There is no background synchronizer or direct sub2api database coupling.
...
long-lived pools. It has no long-lived local daemon. The reset v1
...
- Current compact Codex path:
  A successful new or already-valid `ae-cli login`, and a successful non-dry-run `ae-cli discover` that persisted at least one supported tool configuration, run the same idempotent best-effort activation path. It preserves the stable installation and selected Relay provider, recovers only a missing reporter credential, enables reporting while disabling legacy AE OTel, removes only an exporter whose endpoint and credential still exactly match this installation, and installs machine-level managed hooks without rolling back login or tool configuration on failure. Formal mode neither creates nor requires a v1 baseline. `ae-cli attribution enable` is the advanced recovery entry, not a normal setup step. User-managed OTel is preserved. Managed hooks need no separate `ae-cli init`: canonical-remote resolution stays read-only for known repositories, while reporter-authenticated `not_found` narrowly creates the minimum Repository identity and continues the same event. The automatic path never creates or changes SCM credentials, provider binding, or webhook configuration, and unbound repositories remain reportable. `ae-cli init` remains an explicit fallback-hook/advanced registration command. Codex JSONL always supplies structured mutation evidence. HTTP claims carry exact trusted Request IDs and use matching `sub2api` Token; WebSocket claims carry only normalized model/15-minute JSONL Token aggregates after trusted completion evidence. A mixed turn fails closed. The compact runner uploads digest-minimized v2 claims and deterministic commit evidence, not raw JSONL, WebSocket response IDs, or code. Claude/Kiro and older CLIs retain the existing `tool_usage_events` compatibility path.
- Compact trigger boundary:
  In compact mode, hook-time snapshot collection is skipped and the reporter checkpoint API accepts a strict minimized DTO that excludes `agent_snapshot`, `session_id`, and raw payload fields. `post-commit` and `post-rewrite` first persist small Git triggers, then coalesce a workspace task and opportunistically start the detached runner. Each new v2 commit trigger freezes its Relay provider identity. Retained triggers prevent late Codex evidence from missing its first qualifying commit and allow amend/rebase/squash rewrites to restate the current allocation. Explicit cherry-pick reflog evidence plus stable patch ID creates a non-counting inherited commit reference. One transient machine-wide owner serializes the per-workspace task queue for the active reporting owner, so repository/worktree runners cannot contend on the machine claim ledger. Each workspace receives a bounded five-minute quantum; an exhausted quantum persists exact progress as `yielded` and is immediately selected again without another Git event, while a dead owner lock is recoverable. If the original worktree has been removed, a pre-push or manual sync from another checkout may lend only its Git root to the retained task when server/owner, Repository ID, canonical remote identity, and every exact commit all match and the commit remains reachable from `HEAD` or a Git ref. The original workspace, checkpoint event, commit, provider, and evidence identities never change. Missing or mismatched identity and unreachable commits remain safely queued with local-state diagnostics; recovery never uses cwd, time, patch similarity, or branch-name heuristics. On upgrade, runner/status/doctor activity lazily promotes legacy single-trigger tasks for the same server and reporting owner, freezes a missing provider from that persisted reporting installation, deduplicates exact triggers and progress units, and rebuilds stale scanner progress. Busy, conflicting, and future-version workspaces are deferred without blocking unrelated tasks. The owner terminates when its matching queue is empty and is not a daemon or periodic synchronizer. It applies the 90-day window before reading contents, includes both active and archived Codex sessions, queries trusted SQLite HTTP/WS transport evidence once per pass, streams each source once for all pending commits, and saves digest-only `source × trigger` completion units after claim-state persistence. Empty-candidate sources use bounded progress batches to avoid quadratic rewrites; a batch that finds a claim saves and delivers it before unrelated sources finish. A timeout, process exit, response loss, backend failure, or late source-specific transport event therefore leaves exact resumable work without restarting unrelated sources or duplicating accepted accounting. Scanner-semantics changes bump the progress version so an older completed unit is rebuilt and rescanned once. The retained v1 bucket/revision writer is rejected in formal mode; older non-Codex compatibility keeps the collector, spool, and `tool_usage_events` behavior.
- Reporting durability:
  v2 reporting is at-least-once while local state is writable. Claim-group identities and digests are deterministic, canonical payload conflicts fail closed, and the client deletes only independently acknowledged HTTP Request/calibration values or a still-identical WebSocket group aggregate. Later monotonic JSONL growth reopens an acknowledged WebSocket group. Pending claims and scan progress remain local until accepted. Status and doctor show the current Repository task separately from machine-wide queued/running/yielded/recoverable tasks, terminal conflicts, and tasks within seven days of local expiry; they expose only a safe failure stage/reason, first failure time, and remaining trigger count. Migration and 90-day cleanup remove only local recovery detail and never mutate accepted formal pools or server data. The older hook queue, unresolved queue, tool-usage spool, and dead-letter behavior remains active for the compatibility path; reset v1 bucket/revision state is not formal accounting truth.
...
- Remaining direction:
  observe seven continuous stable days from the replacement #252 Day 0, require at least one later qualifying ordinary-workflow pool, and repeat every final Activity/SCM/health/conservation/zero-v1 gate before cleanup. The earliest possible cleanup time is `2026-08-24T05:32:57.925948Z`; cleanup still requires separate implementation, release, deployment, and production-data authority. Also evaluate compact-trigger retention/claim deadlines from production distributions, migrate Claude/Kiro to the generic pool contract, and decide whether delivery-effectiveness correlation is useful. Cost allocation and individual ranking remain out of scope.
- Active Codex v2 contract:
  [`2026-08-11-codex-commit-token-attribution-v2-design.md`](./superpowers/specs/2026-08-11-codex-commit-token-attribution-v2-design.md) defines mutually exclusive HTTP `relay_official` and WebSocket `codex_local` claim groups, deterministic commit proof, 90-day hot detail, long-lived globally deduplicated usage pools, Usage-backed Activity ratio reads, in-page Repository/PR analysis filters, and administrator-only Repository operations. Production selected the formal HTTP contract at the verified 2026-08-12 cutover and added the WebSocket path in platform `v0.1.0-preview.85`. CLI-only preview.9 repaired trusted-log correlation, preview.10 repaired current Codex 0.147 dual-baseline and stale scan progress, and `ae-cli/v0.2.0-preview.11` preserved single-allocation evidence identity while quarantining unchanged terminal conflicts. The released preview.11 runner drained the retained production task through ordinary managed hooks, recovered exactly 16 late official Requests and `2,773,595` Token into two existing ordinary pools, and left one terminal conflict locally visible. The provisional #252 Day 0 at `2026-08-14T07:22:18.199843Z` was later invalidated by the cumulative personal-denominator defect. Production now serves platform `v0.1.0-preview.87`; exact selected-window readback established replacement Day 0 at `2026-08-17T05:32:57.925948Z`, while the pre-cutover `shadow_v2` pool remains isolated.
