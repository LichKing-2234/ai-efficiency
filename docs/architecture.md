...
schema contraction. Production remains on `v0.1.0-preview.88` until the Phase 2
release and rollout complete. The older sessionless `tool_usage_events` lane
...
- Remaining direction:
  retain the replacement #252 Day 0 as a historical and conservation baseline. Ordinary AI Efficiency PR #319 satisfied the later pool-delta gate with five direct formal pools and `23,210,615` Token; the 2026-08-20 execution-time snapshot also passed Activity, SCM, health, lifecycle, conservation, duplicate/gap, and zero-v1 gates. The operator has authorized implementation, separate releases, deployment, and destructive migration. Repeat every volatile readback immediately before mutation; no fixed elapsed-time wait applies, and a failed gate blocks cleanup until corrected and read back. Also evaluate compact-trigger retention/claim deadlines from production distributions, migrate Claude/Kiro to the generic pool contract, and decide whether delivery-effectiveness correlation is useful. Cost allocation and individual ranking remain out of scope.
