...
- Compact pass ordering:
  CLI preview.16 shipped the Codex 0.149.1 evidence and scanner v5 contract. Current issue #396 code then makes frozen compact v2 scan/persistence/ACK the first operation in a v2-capable workspace pass, before unresolved and workspace checkpoint replay. Both legacy queues remain durable and are still attempted afterward, while their latency can no longer starve formal v2 delivery. Older non-v2 tool-usage passes retain their previous ordering. This ordering repair is not yet released.
