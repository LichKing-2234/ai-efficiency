...
- session detail 读模型会同时返回最近的 `agent_metadata_events`、`session_usage_events` 和 `session_events`
- `agent_metadata_events` 用于展示 agent 侧快照指标（例如 `cached_input_tokens`、`reasoning_tokens`、`credit_usage`）
- `session_usage_events` 则用于展示 proxy/request 级 usage 事实（`input_tokens`、`output_tokens`、`total_tokens`）
- 这两个视角必须保持分离，避免把 agent 快照与单次请求 usage 混为同一种合同
