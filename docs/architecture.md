...
- The embedded SPA now exposes a regular-user `/user` surface for profile summary, provider-aware CLI install/login/discover guidance, and provider-first, group-second credential self-serve driven by the current relay user's allowed groups. User-owned API keys are partially masked in the browser but remain copyable when relay returns the key value. The same page can test the current user's own active provider key through `/api/v1/user/providers/:id/test` using the selected group's platform and a caller-supplied model; the admin Relay Providers surface is CRUD-only and does not expose this test action. `ae-cli discover` now consumes the same user-provider credential surface at `/api/v1/user/providers`, with `/api/v1/providers` kept only as an older backend compatibility fallback.
