...
- Team summary, trend, members, and organization each own an authoritative origin, versioned Redis lane, lease, freshness, and stale-if-error boundary. Summary loads only aggregate stats; Trend loads only the stats and trend points needed for the complete team-total plus bounded Top 12 member and department-comparison series; Members loads complete-range batch stats in at-most-100-user chunks and ranks immutable rows once without acquiring the Trend capability or constructing an organization tree. Organization binds each cached value to one nullable parent and loads only that parent's immediate departments plus complete branch-local ranked direct-member facts, using complete-range batch stats for requested child-subtree aggregates without constructing a recursive tree. These response caches remain independent. Direct Trend reads, complete-range fallback reads, and successful authenticated Personal Usage origins now share one 60-second Redis primitive keyed by deployment namespace, schema, Relay provider ID, persisted provider configuration version, Relay user ID, and normalized start/end/granularity/timezone; requesting actor identity is deliberately absent. Personal Usage writes only the matched authenticated Relay user's reduced date, actual-cost, and optional-token points, and Redis failure remains fail-open. One miss uses the individual origin. Two or more misses prefer Sub2API `users-trend` with `min(max(unique positive requested Relay IDs + 250, 500), 5000)` as the batch limit, filter every row back to the authorized requested IDs, and cache proven empty users only when the result is untruncated. A result containing exactly the limit is possibly truncated, so returned requested users are reused and only unresolved users fall back to individual origins; malformed, failed, or unavailable batches also fall back without caching errors. Identical batch miss sets coordinate across Pods through a token-protected 15-second Redis lease. Individual fallbacks remain behind the existing zero-value-safe provider-wide 24-slot limiter, and cancellation while waiting starts no later origin. No completed per-user trend result remains in Pod memory. The current contract is specified in [Team Usage Redis Batch Trend Design](superpowers/specs/2026-07-20-team-usage-redis-batch-trend-design.md); the July 19 specs remain historical design records. The first-party page starts those four requests independently, so a delayed or failed section does not remove successful siblings, and the trend chart stays behind an async component boundary until a Trend response renders. The temporary compatibility route owns no Relay origin, Redis lane, or cache metric: it resolves one authorized request context, sequentially reads Summary, Trend, and complete Members through their typed lanes, and projects the recursive historical tree from the same scope and Members value without calling paginated Organization branches. It retains the complete historical JSON shape, accepted ineffective `page`/`page_size`, and `Deprecation`, `Sunset`, and successor `Link` headers for the announced compatibility release.
- Pod-local state is classified by purpose rather than by the mere presence of a map or mutex. Embedded frontend raw/lazy-gzip bytes are immutable representation reuse, and `relayruntime.Manager.clients` holds version-bounded reusable Relay clients; neither is a completed Team Usage trend result cache. `readcache.FlightGroup`, origin limiters, and provider/user setup locks coordinate only in-flight work inside one process and retain no completed trend values. The OAuth handler's `codes`, `devices`, and `devicesByUserCode` maps are different: they hold short-lived authorization/device protocol state, so issue, verify, and exchange requests reaching different replicas remain a separate multi-replica correctness risk until that state is shared or routing affinity is guaranteed.
...
- Members resolves only the current authorized scope, Relay identities, and
  complete-range batch stats. It sends at most 100 Relay user IDs per stats call,
  maps selected-window billed usage and token totals into member rows, and ranks the
  complete supported scope once by token total then stable subject identity before
  pagination. It never acquires
  `TeamMemberTrendProvider`, composes Summary or Trend DTOs, or constructs the recursive
...
- The Organization lane selects one authorized branch directly from the flat,
  ...
  immediate departments and exact direct members. To preserve department aggregate
  facts, it resolves only subjects in the requested child subtrees plus the parent's
  direct subjects and loads complete-range stats in batches of at most 100 Relay
  user IDs. Multi-membership members are deduplicated inside each aggregate while
  ...
  external ID, default to 25, and cap at 100; direct members receive dense
...
- The backend OAuth handler still manages short-lived authorization codes and device-flow entries in process-local maps. Unlike immutable frontend bytes, reusable Relay clients, or in-flight coordination primitives, these maps contain protocol state whose cross-replica availability matters; multi-Pod OAuth requires shared state or guaranteed routing affinity and is not addressed by the Team Usage Redis primitive.
...
| Representative scope and team usage | `backend/internal/representativescope`, `backend/internal/readcache`, `backend/internal/teamusage`, `backend/internal/relay` | Resolve representative subtree scope from current directory metadata and member-department memberships, derive and twice-check opaque scope versions, reuse namespace/provider/actor/scope/range-isolated Redis read models with 144-162 second freshness plus bounded stale-if-error and authoritative fallback, serve Summary, bounded Trend, snapshot-bound paged Members, and parent-bound shallow Organization branches from independent typed lanes, share actor-independent 60-second per-provider/version/Relay-user/range Redis trend primitives through two-miss bounded batch origins, a 15-second cross-Pod lease, truncation-aware individual fallback, authenticated Personal Usage write-through, and the existing provider-wide 24-slot individual-origin limiter without retaining completed trend results in Pod memory, retain the complete overview generation only for the legacy adapter, enforce delegated subject visibility and ancestor-only multiplier policy, and persist local `team_usage_rate_multiplier_audits` |
