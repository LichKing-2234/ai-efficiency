...
describe('Settings page composition', () => {
  ...
  test('uses specific reference-style add CTA labels for settings management sections', () => {
    ...
    expect(source).toContain("from './settings-section-add-action'")
    expect(source).toContain('<SettingsSectionAddAction')
    expect(source).toContain("section='ai-services'")
    expect(source).toContain("section='code-platforms'")
    expect(source).toContain("section='advanced-credentials'")
  ...
  test('routes repeated add and refresh CTA buttons through the shared icon-button primitive', () => {
    const source = readFileSync(join(ROOT, 'features/settings/settings-page.tsx'), 'utf8')
    const actionSource = readFileSync(join(ROOT, 'features/settings/settings-section-add-action.tsx'), 'utf8')

    expect(source).toContain("from '@/components/primitives/button-with-icon'")
    expect(source).toContain("from './settings-section-add-action'")
    expect(source).toContain('<SettingsSectionAddAction')
    expect(source).toContain("icon={RefreshCw}")
    expect(actionSource).toContain("from '@/components/primitives/button-with-icon'")
    expect(actionSource).toContain("icon={icon}")
    expect(actionSource).toContain("'ai-services': Layers")
    expect(actionSource).toContain("'code-platforms': Waypoints")
    expect(actionSource).toContain("'advanced-credentials': KeyRound")
