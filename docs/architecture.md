...
| Work items | `backend/internal/workitems` | Auth-scoped pending work counters for sidebar and `/work-items`, including personal AI access setup, pending quota reset approvals, admin quota reset fallback queue count, and current directory offboarding candidate count |
