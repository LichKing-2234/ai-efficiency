...
flowchart LR
    ...
    Redis[("reconstructible read models<br/>Redis")]
...
- PostgreSQL remains authoritative for Work Items state, its persisted cache revision, users, and Relay provider configuration versions. Redis is an optional performance read model for work-item counts and personal usage snapshots; an unavailable Redis never becomes an authorization, mutation, credential, or fresh-quota dependency.
