...
- Current Activity surface:
  `/activity` remains the canonical personal AI Coding Activity surface, with authorized member drill-down at `/activity/members/:user_id` and representative/Admin organization views at `/activity/teams` and `/activity/teams/:team_id`. The legacy v1 routes remain unchanged before cutover. The backend also exposes authorization-revalidated `/api/v1/activity/v2/overview`, `/v2/repositories`, and `/v2/pull-requests` contracts for exact local-date ranges. These v2 reads aggregate compact pools in PostgreSQL, use fixed 20-row signed keyset pages, keep claim and SCM-sync coverage separate, and obtain the denominator only through Personal/Team Usage business services. Their formal ledger epoch is deliberately unset in the production router before cutover, so `shadow_v2` pools cannot change current Activity or readiness.
- Repository surface:
  Browser repository inventory, detail, PR sync, webhook, credential-binding, and mutation routes under `/api/v1/repos` are administrator-only. The three authenticated CLI discovery routes (`ensure-remote`, `resolve-remote`, and `hook-eligible`) and the reporter-token `/api/v1/attribution/repos/resolve-remote` route retain their narrower contracts. The current frontend still presents the pre-v2 repository detail until T09 moves analysis completely into Activity; Activity reads never trigger PR sync.
- Remaining direction:
...
- Implemented v2 shadow usage pools and read model:
  Successful reconciliation materializes official Token components and Request count once into `attribution_usage_pools`, keyed by user, requested model, the sorted counting-commit set, and the non-empty 15-minute UTC usage bucket. `attribution_usage_pool_commits` projects one direct pool to one commit or one shared pool to every counting commit without duplicating the accounting fact. The 90-day hot Request claim keeps only an internal materialized-pool pointer so a later proven allocation can transactionally subtract the old contribution, add the new contribution, and remove an empty old pool. Final unresolved coverage uses a reserved zero-Token `unresolved` pool in the group's first server-received 15-minute bucket because no authoritative upstream model or usage time exists. Long-lived pool rows contain no Request ID, local calibration, API key/account, prompt, response, code, or path. All rows remain `shadow_v2` and do not feed current Activity/readiness before cutover.
- Implemented v2 shadow lineage:
  ...
  The Activity v2 read model counts each formal pool once for scope totals, projects direct/shared Repository participation and many-to-many PR involvement without making those rows additive, and computes daily buckets with PostgreSQL IANA timezone conversion from the pool's upstream 15-minute usage time. Repository and PR search/sort stay server-side. Ratio reads require exact range/timezone/provider/scope coverage, reject stale or partial Usage, bind member cache entries to actor, directory scope, provider version, subject binding, and range, and clamp the numerator to fully elapsed 15-minute pools at the Usage `as_of`. Product DTOs contain no Request identifier or hot-claim detail. Until T12 supplies an explicit formal epoch, these routes return no shadow accounting data.
