...
- PostgreSQL remains authoritative for Work Items state and its persisted cache revision, representative scope guards, users, and Relay provider configuration versions. Redis is an optional performance layer for bounded work-item counts, personal usage snapshots, and representative-scope read models; an unavailable Redis never becomes an authorization, mutation, credential, or fresh-quota dependency.
