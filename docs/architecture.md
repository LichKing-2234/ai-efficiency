...
- Relay Planning Confirm now persists an independent Relationship Operation, all affected-Mapping ownership rows, immutable directional steps, and the initial attempt in one PostgreSQL transaction before any Relay write. Active ownership is partial-unique per Mapping, so concurrent Confirm fails before mutation while unrelated Mappings remain available. Partial execution leaves captured Mapping baselines and revisions unchanged; successful execution promotes every affected baseline with revision compare-and-swap and atomically releases ownership with terminal Operation evidence. Dispatched effects without request-bound proof remain owned and unknown for the later Resume/Restore workflow rather than being replayed by ordinary Confirm.
