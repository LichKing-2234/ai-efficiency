...
- Repository surface:
  Browser repository inventory, detail, PR sync, webhook, credential-binding, and mutation routes under `/api/v1/repos` are administrator-only. The three authenticated CLI discovery routes (`ensure-remote`, `resolve-remote`, and `hook-eligible`) and the reporter-token `/api/v1/attribution/repos/resolve-remote` route retain their narrower contracts. The frontend keeps Repository pages operational-only and presents Token/PR analysis under Activity; Activity reads never trigger PR sync.
- Remaining direction:
