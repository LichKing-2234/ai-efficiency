...
func TestNewClientPreservesSharedBoundedOptions(t *testing.T) {
	...
	if options.MinIdleConns != 4 {
		t.Fatalf("Redis MinIdleConns = %d, want 4", options.MinIdleConns)
	}
	if !options.ContextTimeoutEnabled {
		t.Fatal("ContextTimeoutEnabled = false, want true")
	}
	// go-redis normalizes the configured -1 sentinel to the effective zero-retry value.
