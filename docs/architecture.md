...
- Team trend is also served independently and owns the complete team-total series plus bounded Top 12 member and department-comparison series. Summary, trend, and compatibility requests start independently with the same normalized range; a delayed or failed request does not remove successful siblings. The trend chart stays behind an async component boundary until chartable trend data renders. The compatibility route retains its complete historical JSON shape and emits `Deprecation`, `Sunset`, and successor `Link` headers for the compatibility release.
