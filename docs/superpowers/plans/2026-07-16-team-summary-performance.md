...
**Status:** The original split Summary implementation is integrated into `feat/platform-loading-performance` through PR #153 at `25cf7d6f`; independent cold-origin and compatibility-adapter remediations #164/#172 are also present at the then-current integration head `d2bc2694`. Draft-PR details below are historical delivery evidence. This work is not merged to `main` or production-verified; #136 remains open, and #137 still owns later compatibility removal.
