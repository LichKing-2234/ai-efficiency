...
func createRecoveryFixture(t *testing.T, ctx context.Context, client *ent.Client, observedTarget bool) (*Service, *recoveryProvider, *ent.RelayGroupMapping, *ent.RelationshipOperation) {
	...
	client.RelationshipOperationStep.Create().SetOperationID(operation.ID).SetStepKey("subscription-add-target").SetAction("add").SetRelationshipType("subscription").SetDirection(relationshipoperationstep.DirectionTarget).SetLocalUserID(user.ID).SetRelayUserID(42).SetTargetGroupID(102).SetExpectedResult(map[string]any{"target_active": true, "baseline_active": false}).SetResumeSupported(true).SetRestoreSupported(true).SetLifecycle(relationshipoperationstep.LifecycleDispatched).SaveX(ctx)
	client.RelationshipOperationStep.Create().SetOperationID(operation.ID).SetStepKey("subscription-remove-source").SetAction("remove").SetRelationshipType("subscription").SetDirection(relationshipoperationstep.DirectionTarget).SetLocalUserID(user.ID).SetRelayUserID(42).SetTargetGroupID(101).SetExpectedResult(map[string]any{"target_active": false, "baseline_active": true}).SetResumeSupported(true).SetRestoreSupported(true).SetLifecycle(relationshipoperationstep.LifecycleDispatched).SaveX(ctx)
	...
	client.RelationshipOperationStep.Create().SetOperationID(operation.ID).SetStepKey("target-rename").SetAction("rename").SetRelationshipType("group").SetDirection(relationshipoperationstep.DirectionTarget).SetTargetGroupID(102).SetExpectedResult(map[string]any{"target_name": "Reviewed Target", "baseline_name": "Baseline Target"}).SetResumeSupported(true).SetRestoreSupported(true).SetLifecycle(relationshipoperationstep.LifecycleDispatched).SaveX(ctx)
