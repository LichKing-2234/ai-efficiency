...
- 当前可执行合同应以多轮 `status` 协议为准：`POST /api/v1/tools/discover` 启动、`POST /api/v1/tools/discover/continue` 续跑。
- provider secret 不应进入 LLM prompt；后端只向模型暴露 provider 元数据，最终在 `complete` 响应阶段按 `provider_name` 回填写配置所需的 secret 和 `relay_api_key_id`。
- 本文中早期的一次性 `discovered_tools` 响应示例仅保留为高层说明，不再作为实现合同。
- `config.go` 简化在本文中仍是目标态；当前代码基线尚未完全移除 legacy config 依赖。
...
| api_key_id 存储 | 直接存入 discovered_tools.json 缓存 | 整数 ID 不敏感，避免反向查找的脆弱性（用户可能手动修改工具配置） |
...
│   └── session/
│       └── reporter.go     # session 创建时的 api_key_id 上报逻辑
└── config/
