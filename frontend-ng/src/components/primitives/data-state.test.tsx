...
describe('DataState', () => {
  ...
  test('uses shared centered loading copy and richer error copy structure', () => {
    ...
    expect(source).toContain("<CardContentStack className='p-[24px] text-center' dataSlot='data-state-empty-content'>")
    expect(source).toContain("{description ? <div className='max-w-md text-[12px] text-[var(--ink-3)]' data-slot='data-state-empty-description'>{description}</div> : null}")
