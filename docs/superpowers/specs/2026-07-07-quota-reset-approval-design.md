...
5. Reset success status and the required `reset_succeeded` event commit atomically. If that transaction fails after Relay succeeds, the request remains `approved_resetting` for explicit reconciliation; the service must not automatically replay Relay.
...
10. Reset success does not advance the work-item revision because `approved_resetting` is already outside the actionable set. The succeeded status and `reset_succeeded` event commit in one transaction.
...
14. Reset success finalization likewise uses a fresh independent 5-second context. It atomically commits `approved_reset_succeeded` and `reset_succeeded`, so caller cancellation after Relay success cannot interrupt local finalization.
...
21. A reset-success event failure rolls back `approved_reset_succeeded` to `approved_resetting`, and reset success does not perform another revision change.
...
24. Caller cancellation after Relay success but before local persistence still atomically stores `approved_reset_succeeded` and `reset_succeeded`.
