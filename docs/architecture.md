...
sequenceDiagram
    ...
    CLI->>BE: best-effort reporting installation enrollment + activation
    CLI->>WS: record Codex baseline + install global hooks
    CLI->>Tool: configure trace-safe Codex OTLP
    Dev->>CLI: ae-cli discover
    CLI->>BE: GET /api/v1/user/providers
    CLI->>Tool: configure Codex / Claude / Gemini locally
    CLI->>BE: idempotently retry reporting activation
    Dev->>Tool: run Codex
...
- `ae-cli` owns installation enrollment, automatic compact Codex activation, baseline/state, global or repo-local hook management, edge aggregation, legacy sessionless compatibility, and diagnostics. Successful login and non-dry-run discover run the same idempotent activation path; failure warns without rolling back the primary command, and the explicit attribution-enable command remains an advanced repair entry point. `ae-cli init` remains an explicit repo-registration and fallback-hook command, not a mandatory step for every backend-known repository.
...
flowchart LR
    ...
    subgraph Local["Developer machine"]
        CLI["ae-cli login / discover / sync / doctor"]
...
- Installation and correlation boundary:
  `reporting_installations` stores owner/status/enable flags and only hashes of the independently scoped `aer_*` reporter and `aeo_*` OTLP credentials. Authenticated rotation replaces both credentials and invalidates their prior values; enrolling the same revoked installation as its current owner reactivates it with replacement credentials and disabled flags before normal activation. The only Codex OTLP route is `/api/v1/attribution/otel/v1/traces`; prompt logging is disabled and no logs ingest route exists. The bounded JSON payload is reduced in memory to conversation/Request ID/time/status/error evidence. Raw spans are discarded. Success evidence has a per-item 24-hour shared-cache retention, failure evidence has a separate per-item 30-day retention, later writes do not extend older evidence, and durable buckets retain only correlation quality, count, and digest.
- Current Activity surface:
  `/activity` is the canonical personal AI Coding Activity surface, with authorized member drill-down at `/activity/members/:user_id` and representative/Admin organization views at `/activity/teams` and `/activity/teams/:team_id`. It defaults to 30 days, presents PRs before commits, renders incomplete PR-sync totals as lower bounds, keeps Token out of headline/team metrics, and loads restricted Bucket/Request ID evidence only for the owner or Admin. The personal route alone reads `GET /api/v1/attribution/status` and renders the shared compact reporting guide; member/team routes do not request installation readiness or show setup commands. `/events` and `/attribution` redirect to `/activity` while preserving safe range query state.
- Reporting readiness and setup surface:
  The backend derives one current-user state from existing installation and bucket facts: `not_enrolled`, `revoked`, `disabled`, `waiting_for_data`, or `active`. It returns aggregate counts and optional latest server-accepted bucket `created_at` only, never installation rows or `last_seen_at`; there is no device-management or time-based stale model. Any accepted user bucket establishes active reporting, including unbound/shared allocation. `/user` remains the full setup source but separates AI tool configuration from AI Coding Activity reporting. Tool configuration owns access groups, API keys, connection tests, manual/app import, and provider-specific discover. The reporting section independently owns the platform-specific CLI install/login/generic-discover sequence, reuses the personal Activity guide, and explains automatic activation; normal UI omits explicit enable, hook management, repo init, sync, and upload-status commands, while keeping `attribution status` and `doctor` as collapsed diagnostics. The compact bucket claim is Codex-only.
- Repository surface:
...
| Area | Paths | Responsibility |
| --- | --- | --- |
| Views | `frontend/src/views` | Dashboard, Work Items, personal/member/team AI Coding Activity with personal-only reporting guidance, split AI tool configuration and Codex reporting setup, Activity-first repository detail with lazy Operations, repos, oauth, personal AI Usage, selected-member usage detail, representative Team Overview, admin users with on-demand department browsing and one responsive user-row tree, paginated admin Directory offboarding, and admin/settings pages with immediate affected-mutation count refresh |
