...
- `My Work`: `/`, `/user`, and `/events` provide the ordinary company-user path for personal AI usage, setup, and readable usage records. `/` is now a lifecycle-aware personal home: users without usable AI access, or users with access but no effective usage data yet, land on an expanded setup guide card that pushes them toward `/user`; established users land on the embedded usage dashboard first, with the setup guide collapsed by default. `Usage Records` and `Code Repositories` remain secondary drill-down paths rather than the homepage's primary narrative.
