...
describe('AuthSurface', () => {
  test('renders a centered auth card with shared header and body stack', () => {
    ...
    expect(html).toContain('data-slot="auth-surface-brand"')
  ...
  test('sources auth actions from the shared action group primitive', async () => {
    ...
    expect(source).toContain("data-slot='auth-surface-brand'")
    expect(source).toContain("data-slot='auth-surface-caption'")
    expect(source).toContain("className={cn('grid-paper w-full overflow-hidden border-[var(--ai-line)] shadow-[var(--sh-lg)]', className)}")
    expect(source).toContain("className='border-border border-t px-[18px] py-[12px]'")
