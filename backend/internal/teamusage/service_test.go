...
func TestTrendReturnsHardErrorForAuthorizationFailure(t *testing.T) {
	client := testdb.Open(t)
	createPrimaryRelayProvider(t, client)
	scope := &representativescope.Scope{
		ActorUserID: 1, IsRepresentative: true,
		Subjects: []representativescope.Subject{
	...
	_, err := service.Trend(context.Background(), 1, summaryTestParams())
	if !errors.Is(err, relay.ErrInvalidCredentials) {
		t.Fatalf("Trend() error = %v, want relay.ErrInvalidCredentials", err)
	}
}
