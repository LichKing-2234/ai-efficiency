...
- `Administration`: `/admin/users` and `/settings` are admin-only surfaces. `/admin/users` defaults to user, relay mapping, access status, and centralized sub2api subscription management with selected/current-filter/all-mapped scopes and add/extend/remove operations, while keeping plaintext relay password copy behind an explicit risk confirmation; on mobile it renders selectable user cards rather than a compressed wide table. `/settings` is implemented as task-zone section components for AI Services, Code Platforms, Organization & Login, Deployment & Runtime, and Advanced Credentials, with bilingual primary sections and add/edit dialogs. Deployment apply, rollback, and restart actions require a confirmation step before calling admin APIs.
