...
describe('DirectorySyncSettings', () => {
  ...
  it('renders Directory Sync actions with Element Plus controls', async () => {
  ...
  it('renders safe templates and copies an AI prompt with safety guidance', async () => {
    ...
    expect(wrapper.text()).toContain('department/member declarations form a union')
    await wrapper.get('[data-testid="directory-template-0"]').trigger('click')
    const safeTemplate = wrapper.get('[data-testid="directory-dsl"]').element as HTMLTextAreaElement
    expect(safeTemplate.value).toContain('representative_external_ids:')
    expect(safeTemplate.value).toContain('leader_department_ids:')
    expect(safeTemplate.value).toContain('remove:')
    expect(safeTemplate.value).toContain('member-alice')
    expect(safeTemplate.value).toContain('department-alpha')
    ...
    expect(document.body.textContent).toContain('AI prompt copied')
    ...
    expect(prompt).toContain('representative_external_ids:')
    expect(prompt).toContain('leader_department_ids:')
    expect(prompt).toContain('remove:')
    expect(prompt).toContain('remove first, append second')
    expect(prompt).toContain('representative scope is the union of department and member declarations')
    expect(prompt).toContain('missing or ambiguous exact targets fail preview and apply')
    ...
    expect(prompt).toContain('remove first, append second')
    expect(prompt).toContain('union of department and member declarations')
    expect(prompt).toContain('at most 100 department overrides and 100 member overrides')
    expect(prompt).toContain('missing or ambiguous exact targets fail preview and apply')
  ...
  it('shows Element Plus feedback when copying the AI prompt fails', async () => {
    ...
    expect(document.body.querySelector('.el-message--error')).not.toBeNull()
  })
