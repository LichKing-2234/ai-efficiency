...
- `Relay Planning`: `/admin/relay-planning` is an explicit admin-only department x Platform workflow. Its initial preview automatically recommends a fixed target Group count from indivisible source members and keeps Template Group (copy configuration) separate from the optional Migration Source Group (old subscription/API Key migration). After Preview, an administrator can search any local user, verify that user's identity through the selected Relay Provider, assign the user to one explicit Target, and choose one same-Platform Source or target-only addition. Replan is entered from one mapping, preserves stable target Group IDs and saved department relationships, and never resizes or silently reshuffles Groups. Mapping maintenance reads and adopts privacy-safe per-target Account relationships, applies reviewed same-Platform Account pools only through Confirm, and warns about drift, multiple Accounts, reuse, unavailable Groups, unmanaged members, and cross-mapping conflicts. Explicit removal, `Move Here`, `Add Additionally`, and unmanaged-member adoption remain admin edits that require final confirmation; department changes never trigger them automatically. Every maintenance Preview carries a compact versioned relationship fingerprint with separate Group, Account, mapping/retry, Relay identity, subscription, and API-Key hashes. Confirm rejects stale categories before any Relay write, while usage changes remain advisory. Each mapping retains per-Group/member operation state and errors so failed move/removal steps can be reopened and retried. Destination and source mapping updates commit atomically and return per-mapping persistence results. The read-only preview reuses a valid schema-v3 Team Usage prewarm generation for provider-wide 30-day usage and falls back to exact Relay sources when that generation is absent or invalid; current subscription membership and migratable API Key facts remain authoritative per candidate. There is no background synchronizer or direct sub2api database coupling.
